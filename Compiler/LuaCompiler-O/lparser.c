...
static void funcargs (LexState *ls, expdesc *f) {
  ...
  switch (ls->t.token) {
    ...
    case TK_HASHSTRING: {  /* funcargs -> STRING */
  ...
  if (testnext(ls, ',')) {  /* assignment -> `,' primaryexp assignment */
    ...
    assignment(ls, &nv, nvars+1);
      ...
      case TK_ADDSELF: {
          ...
          e.t = 0;
          e.f = 0;
          luaK_posfix(ls->fs, OPR_ADD, &c, &e);
          luaK_storevar(ls->fs, &lh->v, &c);
          return;
      }
      case TK_PLUSEQ: {
          expdesc c = lh->v;
      ...
      case TK_CONCATEQ: {
          ...
          luaX_next(ls);
          explist1(ls, &e);
          luaK_posfix(ls->fs, OPR_CONCAT, &c, &e);
          luaK_storevar(ls->fs, &lh->v, &c);
          return;
      }
      }
...
static void getlocalattribute(LexState* ls) {
    /* ATTRIB -> ['<' Name '>'] */
    if (testnext(ls, '<')) {
